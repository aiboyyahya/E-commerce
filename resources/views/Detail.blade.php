    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-16">
        <div class="bg-white rounded-3xl shadow-md border border-gray-200 overflow-hidden">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 p-10">
                <div class="space-y-8">
                    <div class="aspect-square bg-gray-50 rounded-3xl flex items-center justify-center border border-gray-200 shadow-inner">
                        <img id="main-product-image"
                            src="{{ $product->image ? asset('storage/' . $product->image) : 'https://via.placeholder.com/500x500' }}"
                            alt="{{ $product->product_name }}"
                            class="object-contain w-full h-full rounded-2xl p-6">
                    </div>
                ...
                <div class="flex flex-col max-w-lg border border-gray-200 rounded-3xl p-10 shadow-sm bg-white sticky top-10 h-fit space-y-8">
                    ...
                    <div class="flex items-center mt-4 space-x-2">
                        <div class="flex items-center text-yellow-400">
                            ...
                            @for ($i = 1; $i <= 5; $i++)
                                <svg class="w-5 h-5 {{ $i <= $avg ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.947a1 1 0 00.95.69h4.148c.969 0 1.371 1.24.588 1.81l-3.36 2.44a1 1 0 00-.364 1.118l1.287 3.948c.3.921-.755 1.688-1.538 1.118l-3.36-2.44a1 1 0 00-1.175 0l-3.36 2.44c-.783.57-1.838-.197-1.538-1.118l1.286-3.947a1 1 0 00-.364-1.118L2.037 9.374c-.783-.57-.38-1.81.588-1.81h4.148a1 1 0 00.95-.69l1.286-3.947z"/>
                                </svg>
                            @endfor
                        </div>
                        <span class="text-gray-600 text-sm font-medium">({{ $ratingCount ?? 0 }} ulasan) • rata-rata {{ number_format($avgRating ?? 0, 1) }}</span>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6 space-y-6 text-gray-700 text-sm">
                        <div class="flex items-start gap-4">
                            <svg class="w-6 h-6 text-orange-500 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
