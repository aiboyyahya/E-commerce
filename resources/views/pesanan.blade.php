    ...
    <div class="max-w-6xl mx-auto px-6 lg:px-12">
        ...
        <div class="mb-8 border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <a href="{{ route('orders', ['status' => 'all']) }}" class="tab-link {{ isset($status) && $status == 'all' ? 'border-orange-500 text-orange-600' : 'border-gray-100 text-gray-500 hover:text-gray-700 hover:border-gray-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm" data-status="all">Semua</a>
                <a href="{{ route('orders', ['status' => 'pending']) }}" class="tab-link {{ isset($status) && $status == 'pending' ? 'border-orange-500 text-orange-600' : 'border-gray-100 text-gray-500 hover:text-gray-700 hover:border-gray-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm" data-status="pending">Belum Bayar</a>
                <a href="{{ route('orders', ['status' => 'packing']) }}" class="tab-link {{ isset($status) && $status == 'packing' ? 'border-orange-500 text-orange-600' : 'border-gray-100 text-gray-500 hover:text-gray-700 hover:border-gray-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm" data-status="packing">Dikemas</a>
                <a href="{{ route('orders', ['status' => 'sent']) }}" class="tab-link {{ isset($status) && $status == 'sent' ? 'border-orange-500 text-orange-600' : 'border-gray-100 text-gray-500 hover:text-gray-700 hover:border-gray-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm" data-status="sent">Dikirim</a>
                <a href="{{ route('orders', ['status' => 'done']) }}" class="tab-link {{ isset($status) && $status == 'done' ? 'border-orange-500 text-orange-600' : 'border-gray-100 text-gray-500 hover:text-gray-700 hover:border-gray-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm" data-status="done">Selesai</a>
            </nav>
        </div>
                            ...
                            <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-mono text-blue-800">{{ $transaction->tracking_number }}</p>
                                    </div>
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="mt-6 flex justify-between items-center border-t pt-5">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    @if ($transaction->status == 'done')
                                        @php
                                            $allRated = $transaction->items->every(function ($item) use ($transaction) {
                                        ...
                                        @endphp
                                        @if (!$allRated)
                                            <a href="{{ route('ratings.create', ['product_id' => $transaction->items->first()->product->id, 'transaction_id' => $transaction->id]) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition shadow-sm">
                                                Kirim Ulasan
                                            </a>
                                        @endif
                                    @endif
                    ...
                    <div class="empty-state {{ $statusKey }}" data-status="{{ $statusKey }}">
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 text-center">
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                @switch($statusKey)
                                    @case('pending') Belum ada pesanan yang belum dibayar. @break
                                    @case('packing') Belum ada pesanan yang sedang dikemas. @break
                                    @case('sent') Belum ada pesanan yang sedang dikirim. @break
                                    @case('done') Belum ada pesanan yang telah selesai. @break
                                @endswitch
                            </p>
                        </div>
                    </div>
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', e => {
            ...
            sections.forEach(section => {
                section.classList.toggle('hidden', !(status === 'all' ? section.dataset.status === 'all' : section.dataset.status === status));
            });
            emptyStates.forEach(emptyState => {
                emptyState.classList.toggle('hidden', !(status !== 'all' && emptyState.dataset.status === status));
            });
        });
    });
